...
const AppState = {
  ...
  async loadData() {
    try {
      // Read data from embedded JSON in HTML (avoids CORS issues with file://)
      const dataElement = document.getElementById('commission-data');
      if (!dataElement) {
        throw new Error('commission-data element not found');
      }
